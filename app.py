        ...
        function procesarPagoAPI(id, n, m) {
            fetch('/api/marcar_pago/' + id + '/' + n + '?monto=' + m).then(res => res.json()).then(d => {
                if(d.status === 'ok') {
                    ...
                    // CORRECCIÓN QUIRÚRGICA: Limpieza total por expresión regular de comillas en teléfonos
...
def api_marcar_pago(cliente_id, num_cuota):
    ...
    recibo = {"cliente": cliente["nombre"], "cuota": num_cuota, "monto": monto_ingresado, "saldo": saldo_restante, "telefono": cliente["telefono"], "mensaje_ws": texto_ws}
    conn.close()
